...
def handle_url(message):
    ...
    try:
        ...
        if result.get("status") != "success":
            return bot.reply_to(message, "Masukkan Link PoopHD yg valid")
        ...
        if thumbnail_url:
            try:
                ...
                thumb = io.BytesIO(resp.content)
                thumb.name = 'thumbnail.jpg'
            except Exception as err:
                print(f"Gagal mengunduh thumbnail") #: {err}

        # Kirim video atau dokumen sesuai size, dengan thumbnail jika ada
